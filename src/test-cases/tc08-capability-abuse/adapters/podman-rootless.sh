...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(create_host_probe_dir "tc08-podman-helper")" || cld6001_block "precondition_failed" "Failed to create controlled host helper bundle for TC08"
...
run_podman_probe() {
    ...
    local output=""
    local status=0
    local -a podman_args=(run --rm -i --cap-drop=all -v "$HOST_HELPER_DIR:/host-helper:ro")

    if [ "$label" = "host-pid" ]; then
        podman_args+=(--pid=host)
    fi
    if [ -n "$cap" ]; then
        podman_args+=(--cap-add="$cap" -e "CAP_UNDER_TEST=$cap")
    else
        podman_args+=(-e CAP_UNDER_TEST=negative-control)
    fi

    set +e
    output="$(podman "${podman_args[@]}" "$IMAGE" sh -c "$probe_script" 2>&1)"
    status=$?
    set -e
...
print_podman_result() {
    ...
    echo "$description"
    if output="$(run_podman_probe "$label" "$cap" "$probe_script")"; then
        printf '%s\n' "$output"
    else
        printf '%s\n' "$output"
        echo "$block_message"
...
test_dac_override() {
    ...
    print_podman_result "Negative control: no added capability" "helper" "" "$probe_script" "BLOCK: rootless Podman failed before DAC negative control completed"
    print_podman_result "Positive case: granting CAP_DAC_OVERRIDE" "helper" "CAP_DAC_OVERRIDE" "$probe_script" "BLOCK: rootless Podman refused CAP_DAC_OVERRIDE helper read attempt"
    echo ""
}
...
test_sys_admin() {
    ...
    print_podman_result "Negative control: no added capability" "helper" "" "$probe_script" "BLOCK: rootless Podman failed before SYS_ADMIN negative control completed"
    print_podman_result "Positive case: granting CAP_SYS_ADMIN" "helper" "CAP_SYS_ADMIN" "$probe_script" "BLOCK: rootless Podman refused CAP_SYS_ADMIN helper remount"
    echo ""
}
...
test_mknod() {
    ...
    print_podman_result "Negative control: no added capability" "helper" "" "$probe_script" "BLOCK: rootless Podman failed before MKNOD negative control completed"
    print_podman_result "Positive case: granting CAP_MKNOD" "helper" "CAP_MKNOD" "$probe_script" "BLOCK: rootless Podman refused CAP_MKNOD device-node attempt"
    echo ""
}
...
test_sys_ptrace() {
    ...
    print_podman_result "Negative control: no added capability" "host-pid" "" "$probe_script" "BLOCK: rootless Podman does not expose a host PID target for CAP_SYS_PTRACE"
    print_podman_result "Positive case: granting CAP_SYS_PTRACE" "host-pid" "CAP_SYS_PTRACE" "$probe_script" "BLOCK: rootless Podman does not expose a host PID target for CAP_SYS_PTRACE"
    echo ""
}
