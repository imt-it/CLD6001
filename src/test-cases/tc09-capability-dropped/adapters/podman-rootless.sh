...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(create_host_probe_dir "tc09-podman-helper")" || cld6001_block "precondition_failed" "Failed to create controlled host helper bundle for TC09"
...
all_dropped_output="$(podman run --rm -i --cap-drop=all -v "$HOST_HELPER_DIR:/host-helper:ro" "$IMAGE" sh -c '
...
selective_cap_output="$(podman run --rm -i --cap-drop=all --cap-add=NET_BIND_SERVICE -v "$HOST_HELPER_DIR:/host-helper:ro" "$IMAGE" sh -c '
