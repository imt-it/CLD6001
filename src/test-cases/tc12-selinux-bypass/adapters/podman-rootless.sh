...
tc12_podman_policy_unreadable() {
    local output="${1:-}"

    grep -Eq 'open /etc/containers/policy\.json: permission denied' <<<"$output"
}

block_for_tc12_tooling_image_failure() {
    local output="${1:-}"

    [ -z "$output" ] || printf '%s\n' "$output" >&2

    if tc12_podman_policy_unreadable "$output"; then
        cld6001_block "podman_policy_unreadable" "Podman rootless cannot read /etc/containers/policy.json; fix the host Podman policy file permissions before rerunning TC12"
    fi

    cld6001_block "tc12_tooling_image_unavailable" "Failed to build the TC12 tooling image"
}
...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(create_host_probe_dir "tc12-podman-helper")" || cld6001_block "precondition_failed" "Failed to create controlled host helper bundle for TC12"
...
require_tc12_container_tooling() {
    ...
    if [ -n "${RUNNER_TC12_TOOLING_IMAGE:-}" ]; then
        cld6001_ensure_image podman "$TC12_TOOLING_IMAGE" || exit 1
    elif ! inspect_output="$(podman image inspect "$TC12_TOOLING_IMAGE" 2>&1)"; then
        if tc12_podman_policy_unreadable "$inspect_output"; then
            block_for_tc12_tooling_image_failure "$inspect_output"
        fi
...
EOF
            ...
            block_for_tc12_tooling_image_failure "$build_output"
        fi
    fi

    missing="$(podman run --rm -e TOOLING_MISSING="$TOOLING_MISSING" --entrypoint sh "$TC12_TOOLING_IMAGE" -c '
