...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(create_host_probe_dir "tc12-docker-rootless-helper")" || cld6001_block "precondition_failed" "Failed to create controlled host helper bundle for TC12"
...
docker_rootless_label_override_unsupported() {
    local output="${1:-}"

    grep -Eqi -- '--security-opt label(=| )|SELinux.*not supported in rootless mode|security[- ]opt.*not supported in rootless mode' <<<"$output"
}
...
run_case() {
    ...
    if docker_rootless_label_override_unsupported "$output"; then
        printf '%s\n' "$output"
        printf 'docker-rootless SELinux limitation: BLOCK (%s cannot exercise %s because --security-opt label=... is not supported in rootless mode)\n' "$case_label" "$security_opt"
        return 0
    fi

    printf '%s\n' "$output" >&2
    cld6001_block "tc12_rootless_probe_launch_failed" "Docker rootless failed to launch the TC12 ${case_label} probe"
}
...
echo "Rootless Docker note: TC12 records blocked launch observations when SELinux label overrides are unsupported in rootless mode."
echo ""

echo "--- Test 1: Context manipulation ---"
context_output="$(run_case "label=type:unconfined_t" "$TC12_CONTEXT_SCRIPT" "context manipulation")"
printf '%s\n' "$context_output" | tee "${TEST_RESULTS_DIR}/tc12-test1.txt"

echo "--- Test 2: Policy manipulation ---"
policy_output="$(run_case "label=type:container_init_t" "$TC12_POLICY_SCRIPT" "policy manipulation")"
printf '%s\n' "$policy_output" | tee "${TEST_RESULTS_DIR}/tc12-test2.txt"

echo "--- Test 3: Transition manipulation ---"
transition_output="$(run_case "label=type:container_t" "$TC12_TRANSITION_SCRIPT" "transition manipulation")"
printf '%s\n' "$transition_output" | tee "${TEST_RESULTS_DIR}/tc12-test3.txt"

echo "--- Test 4: File label manipulation ---"
label_output="$(run_case "label=level:s0:c100,c200" "$TC12_LABEL_SCRIPT" "file label manipulation")"
