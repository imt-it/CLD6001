...
RESULTS_FILE="${TEST_RESULTS_DIR}/tc03-results.txt"
: > "$RESULTS_FILE"
TC03_RUNTIME_ID="${RUNNER_RUNTIME_ID:-podman-rootless}"
...
cld6001_ensure_image podman "$IMAGE" || exit 1

echo "--- Test 3a: Cgroup Freezer Constraint Bypass Assessment ---"
podman run --rm -i "$IMAGE" sh -c "$(tc03_freezer_probe_script "$TC03_RUNTIME_ID")" 2>&1 | tee -a "$RESULTS_FILE"

echo "--- Test 3a-ii: Cgroup Hierarchy Escape Probe ---"
podman run --rm -i "$IMAGE" sh -c "$(tc03_cgroup_hierarchy_probe_script)" 2>&1 | tee -a "$RESULTS_FILE"

echo "--- Test 3b: Memory Constraint Enforcement Assessment ---"
podman run --rm --memory=64m -i "$IMAGE" sh -c '
echo "Memory limit test:"
echo "Cgroup memory limit (own view):"
cat /sys/fs/cgroup/memory.max 2>/dev/null || echo "BLOCKED: memory limit unreadable"
...
podman run --rm --cpus=0.5 -i "$IMAGE" sh -c '
echo "CPU limit test:"
echo "Cgroup CPU limit (own view):"
cat /sys/fs/cgroup/cpu.max 2>/dev/null || echo "BLOCKED: cpu limit unreadable"
...
set +e
podman run --rm --pids-limit=10 -i "$IMAGE" sh -c '
echo "PID limit test:"
echo "Cgroup PID limit (own view):"
cat /sys/fs/cgroup/pids.max 2>/dev/null || echo "BLOCKED: pids limit unreadable"
