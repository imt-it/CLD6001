...
TC23_RUNTIME_ID="${RUNNER_RUNTIME_ID:-podman-rootless}"
...
setup_multi_container_env() {
    ...
    if ! startup_output="$(podman run --rm -d --name "$attacker_container" -v "$SHARED_DIR:/shared:rw" alpine sh -c '
    ...
    if ! startup_output="$(podman run --rm -d --name "$victim_container" -v "$SHARED_DIR:/shared:rw" alpine sh -c '
        echo "Setting up victim container..."
        while true; do sleep 1; done
    ' 2>&1)"; then
        printf 'Failed to start victim container: %s\n' "$startup_output" >&2
        podman rm -f "$attacker_container" >/dev/null 2>&1 || true
        return 1
    fi

    sleep 2

    if podman ps --format '{{.Names}}' | grep -q "$attacker_container" && \
       podman ps --format '{{.Names}}' | grep -q "$victim_container"; then
        printf '%s|%s\n' "$attacker_container" "$victim_container"
        return 0
    fi

    printf 'Failed to verify multi-container environment startup\n' >&2
    podman rm -f "$attacker_container" "$victim_container" >/dev/null 2>&1 || true
    return 1
}
...
test_shared_layers() {
    ...
    attacker_layers=$(podman inspect "$attacker_container" --format '{{json .GraphDriver.Data}}' 2>/dev/null || true)
    victim_layers=$(podman inspect "$victim_container" --format '{{json .GraphDriver.Data}}' 2>/dev/null || true)
...
if [ -n "$attacker_container" ]; then
    podman rm -f "$attacker_container" >/dev/null 2>&1
fi
if [ -n "$victim_container" ]; then
    podman rm -f "$victim_container" >/dev/null 2>&1
fi
