...
RESULTS_FILE="${TEST_RESULTS_DIR}/tc11-results.txt"
HOST_HELPER_DIR=""
...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(create_host_probe_dir "tc11-docker-helper")" || cld6001_block "precondition_failed" "Failed to create controlled host helper bundle for TC11"
...
require_selinux_mode_change_privilege() {
    ...
    getsebool container_transition 2>/dev/null || true
}
