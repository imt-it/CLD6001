...
run_probe_capture() {
    ...
    docker run --rm "$@" -i -e TC01_PROBE_LABEL="$probe_label" "$IMAGE" sh -eu -c "$TC01_PROBE_SCRIPT" 2>&1
}
...
if [ "$baseline_status" -ne 0 ]; then
    error "TC01 negative control failed to execute"
    exit 1
fi

if [ "$attack_status" -ne 0 ]; then
    error "TC01 privileged abuse path failed to execute"
    exit 1
fi

if contains_host_success "$baseline_output"; then
    error "TC01 negative control unexpectedly reached host-scoped resources"
    exit 1
fi

if ! contains_host_success "$attack_output"; then
    error "TC01 privileged abuse path did not demonstrate host reachability"
    exit 1
fi

echo ""
echo "---"
ok "TC01 negative control stayed container-scoped"
ok "TC01 privileged abuse path demonstrated direct host reachability"
