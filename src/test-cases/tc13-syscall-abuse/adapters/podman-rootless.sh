...
echo "--- Default seccomp profile ---"
{
  ...
  if ! podman run --rm -i --cap-add=SYS_ADMIN -e CONTAINER_PROBE_TMP_DIR="$CONTAINER_PROBE_TMP_DIR" "$PROBE_IMAGE" python3 -c "$SYSCALL_PROBE"; then
...
echo "--- Unconfined profile ---"
{
  ...
  echo "Capabilities: SYS_ADMIN (to keep mount/unshare outcomes seccomp-sensitive)"
  if ! podman run --rm -i --cap-add=SYS_ADMIN --security-opt seccomp=unconfined -e CONTAINER_PROBE_TMP_DIR="$CONTAINER_PROBE_TMP_DIR" "$PROBE_IMAGE" python3 -c "$SYSCALL_PROBE"; then
