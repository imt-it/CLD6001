...
create_host_helper_bundle() {
    HOST_HELPER_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tc15-podman-helper.XXXXXX")"
...
validate_user_namespace_protection() {
    ...
    echo "Expected: keep-id reduces host-root equivalence and controlled helper access stays blocked"
    return 1
}
...
echo "User namespace mapping: keep-id"
...
cat > "${TEST_RESULTS_DIR}/tc15-results.txt" <<EOF
${mapping_output}
...
if validate_user_namespace_protection "$mapping_output" "$privilege_output" "$escape_output"; then
    ...
    echo "Results saved to ${TEST_RESULTS_DIR}/tc15-results.txt"
    exit 0
fi
